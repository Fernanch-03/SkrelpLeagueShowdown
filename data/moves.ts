...
export const Moves: {[moveid: string]: MoveData} = {
	...
	ceaselessedge: {
		...
		priority: 0,
		flags: {contact: 1, protect: 1, mirror: 1},
		volatileStatus: 'darksplinter',
		condition: {
			duration: 3,
			onStart(target) {
				this.add('-start', target, 'move: ceaselessedge Edge');
			},
			onResidualOrder: 8,
			onResidual(pokemon) {
				const target = this.getAtSlot(pokemon.volatiles['darksplinter'].sourceSlot);
				if (!target || target.fainted || target.hp <= 0) {
					this.debug('No target found');
					return;
				}
				const typeMod = this.clampIntRange(pokemon.runEffectiveness(this.dex.getActiveMove('ceaselessedge')), -6, 6);
				cthis.damage(pokemon.maxhp * Math.pow(2, typeMod) / 8, pokemon, target);
			},
		},
		critRatio: 1,
		secondary: null,
	...
	instruct: {
		...
		onHit(target, source) {
			...
			const noInstruct = [
				'assist', 'beakblast', 'belch', 'bide', 'celebrate', 'copycat', 'dynamaxcannon', 'focuspunch', 'iceball', 'instruct', 'kingsshield', 'mefirst', 'metronome', 'mimic', 'mirrormove', 'naturepower', 'obstruct', 'outrage','raginfury', 'petaldance', 'rollout', 'shelltrap', 'sketch', 'sleeptalk', 'struggle', 'thrash', 'transform', 'uproar',
			];
			if (
	...
	powershift: {
		...
		condition: {
			onStart(pokemon) {
				this.add('-start', pokemon, 'Power Shif');
				const newatk = pokemon.storedStats.def;
				const newdef = pokemon.storedStats.atk;
				pokemon.storedStats.atk = newatk;
				pokemon.storedStats.def = newdef;
			...
			onEnd(pokemon) {
				this.add('-end', pokemon, 'Power Shif');
				const newatk = pokemon.storedStats.def;
				const newdef = pokemon.storedStats.atk;
				pokemon.storedStats.atk = newatk;
				pokemon.storedStats.def = newdef;
			...
			onRestart(pokemon) {
				pokemon.removeVolatile('Power Shif');
			},
		},
		secondary: null,
		target: "self",
	...
	/*idk if this works, but i will keep it for now. May look futher on the future*/
	stoneaxe: {
		num: 830,
		accuracy: 90,
		basePower: 65,
		...
		priority: 0,
		flags: {contact: 1, protect: 1, mirror: 1},
		volatileStatus: 'stonesplinter',
		condition: {
			duration: 3,
			onStart(target) {
				this.add('-start', target, 'move: Stone Axe');
			},
			onResidualOrder: 8,
			onResidual(pokemon) {
				const target = this.getAtSlot(pokemon.volatiles['stonesplinter'].sourceSlot);
				if (!target || target.fainted || target.hp <= 0) {
					this.debug('No target found');
					return;
				}
				const typeMod = this.clampIntRange(pokemon.runEffectiveness(this.dex.getActiveMove('stoneaxe')), -6, 6);
				cthis.damage(pokemon.maxhp * Math.pow(2, typeMod) / 8, pokemon, target);
			},
		},
		critRatio: 1,
...
Taken on hold until futher testing:
845	Ceaseless Edge	Dark	Physical	15	65	90	VIII ------ probably works
829	Power Shift	Normal	Status	10	—	—	VIII ------ probably works
830	Stone Axe	Rock	Physical	15	65	90	VIII ------ probably works
832	Mystical Power	Psychic	Special	10	70	90	VIII
843	Triple Arrows	Fighting	Physical	15	50	100	VIII ------ probably works
